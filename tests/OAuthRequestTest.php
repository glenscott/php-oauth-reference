...
class OAuthRequestTest extends PHPUnit_Framework_TestCase {	
	...
	public function testCreateRequestFromConsumerAndToken() {
		...
		$request = Request::from_consumer_and_token($cons, $token, 'POST', 'http://example.com');
		...
		$request = Request::from_consumer_and_token($cons, $token, 'POST', 'http://example.com', array('oauth_nonce'=>'foo'));
		$this->assertEquals('foo', $request->get_parameter('oauth_nonce'));
		
		$request = Request::from_consumer_and_token($cons, NULL, 'POST', 'http://example.com', array('oauth_nonce'=>'foo'));
		...
		$request = Request::from_consumer_and_token($cons, $token, 'POST', 'http://example.com/?foo=bar');
		$this->assertEquals('http://example.com/', $request->get_normalized_http_url());
	...
	public function testBuildRequestFromPost() {
		...
		$this->assertEquals(array('foo'=>'bar','baz'=>'blargh'), Request::from_request()->get_parameters(), 'Failed to parse POST parameters');
	}
	
	public function testBuildRequestFromGet() {
		OAuthTestUtils::build_request('GET', 'http://testbed/test?foo=bar&baz=blargh');		
		$this->assertEquals(array('foo'=>'bar','baz'=>'blargh'), Request::from_request()->get_parameters(), 'Failed to parse GET parameters');
	}
	...
	public function testBuildRequestFromHeader() {
		...
		$this->assertEquals(array('oauth_foo'=>'bar','oauth_baz'=>'bla,rgh'), Request::from_request()->get_parameters(), 'Failed to split auth-header correctly');
	}
	...
	public function testHasProperParameterPriority() {
		...
		$this->assertEquals('header', Request::from_request()->get_parameter('oauth_foo'), 'Loaded parameters in with the wrong priorities');		

		OAuthTestUtils::build_request('POST', 'http://testbed/test?oauth_foo=get', 'oauth_foo=post');
		$this->assertEquals('post', Request::from_request()->get_parameter('oauth_foo'), 'Loaded parameters in with the wrong priorities');		

		OAuthTestUtils::build_request('POST', 'http://testbed/test?oauth_foo=get');
		$this->assertEquals('get', Request::from_request()->get_parameter('oauth_foo'), 'Loaded parameters in with the wrong priorities');				
	}
	
	public function testNormalizeHttpMethod() {
		OAuthTestUtils::build_request('POST', 'http://testbed/test');
		$this->assertEquals('POST', Request::from_request()->get_normalized_http_method(), 'Failed to normalize HTTP method: POST');

		OAuthTestUtils::build_request('post', 'http://testbed/test');
		$this->assertEquals('POST', Request::from_request()->get_normalized_http_method(), 'Failed to normalize HTTP method: post');

		OAuthTestUtils::build_request('GET', 'http://testbed/test');
		$this->assertEquals('GET', Request::from_request()->get_normalized_http_method(), 'Failed to normalize HTTP method: GET');

		OAuthTestUtils::build_request('PUT', 'http://testbed/test');
		$this->assertEquals('PUT', Request::from_request()->get_normalized_http_method(), 'Failed to normalize HTTP method: PUT');
	}
	...
	public function testNormalizeParameters() {
		...
		$this->assertEquals( 'name=', Request::from_request()->get_signable_parameters());

		OAuthTestUtils::build_request('POST', 'http://testbed/test', 'a=b');
		$this->assertEquals( 'a=b', Request::from_request()->get_signable_parameters());
		
		OAuthTestUtils::build_request('POST', 'http://testbed/test', 'a=b&c=d');
		$this->assertEquals( 'a=b&c=d', Request::from_request()->get_signable_parameters());
		
		OAuthTestUtils::build_request('POST', 'http://testbed/test', 'a=x%21y&a=x+y');
		$this->assertEquals( 'a=x%20y&a=x%21y', Request::from_request()->get_signable_parameters());
		
		OAuthTestUtils::build_request('POST', 'http://testbed/test', 'x%21y=a&x=a');
		$this->assertEquals( 'x=a&x%21y=a', Request::from_request()->get_signable_parameters());
		
		OAuthTestUtils::build_request('POST', 'http://testbed/test', 'a=1&c=hi there&f=25&f=50&f=a&z=p&z=t');
		$this->assertEquals( 'a=1&c=hi%20there&f=25&f=50&f=a&z=p&z=t', Request::from_request()->get_signable_parameters());
	}
	
	public function testNormalizeHttpUrl() {
		OAuthTestUtils::build_request('POST', 'http://example.com');
		$this->assertEquals('http://example.com', Request::from_request()->get_normalized_http_url());
		
		OAuthTestUtils::build_request('POST', 'https://example.com');
		$this->assertEquals('https://example.com', Request::from_request()->get_normalized_http_url());
		
		// Tests that http on !80 and https on !443 keeps the port
		OAuthTestUtils::build_request('POST', 'http://example.com:8080');
		$this->assertEquals('http://example.com:8080', Request::from_request()->get_normalized_http_url());
		
		OAuthTestUtils::build_request('POST', 'https://example.com:80');
		$this->assertEquals('https://example.com:80', Request::from_request()->get_normalized_http_url());
		
		OAuthTestUtils::build_request('POST', 'http://example.com:443');
		$this->assertEquals('http://example.com:443', Request::from_request()->get_normalized_http_url());
	...
	public function testBuildPostData() {
		OAuthTestUtils::build_request('POST', 'http://example.com');
		$this->assertEquals('', Request::from_request()->to_postdata());

		OAuthTestUtils::build_request('POST', 'http://example.com', 'foo=bar');
		$this->assertEquals('foo=bar', Request::from_request()->to_postdata());
	...
	public function testBuildUrl() {
		OAuthTestUtils::build_request('POST', 'http://example.com');
		$this->assertEquals('http://example.com', Request::from_request()->to_url());

		OAuthTestUtils::build_request('POST', 'http://example.com', 'foo=bar');
		$this->assertEquals('http://example.com?foo=bar', Request::from_request()->to_url());
	...
	public function testConvertToString() {
		OAuthTestUtils::build_request('POST', 'http://example.com');
		$this->assertEquals('http://example.com', (string) Request::from_request());

		OAuthTestUtils::build_request('POST', 'http://example.com', 'foo=bar');
		$this->assertEquals('http://example.com?foo=bar', (string) Request::from_request());
	...
	public function testBuildHeader() {
		OAuthTestUtils::build_request('POST', 'http://example.com');
		$this->assertEquals('Authorization: OAuth', Request::from_request()->to_header());
		$this->assertEquals('Authorization: OAuth realm="test"', Request::from_request()->to_header('test'));
		...
		$this->assertEquals('Authorization: OAuth oauth_test="foo"', Request::from_request()->to_header());
		$this->assertEquals('Authorization: OAuth realm="test",oauth_test="foo"', Request::from_request()->to_header('test'));
		...
		$this->assertEquals('Authorization: OAuth oauth_foo="bar",oauth_baz="bla%2Crgh"', Request::from_request()->to_header());
		$this->assertEquals('Authorization: OAuth realm="test",oauth_foo="bar",oauth_baz="bla%2Crgh"', Request::from_request()->to_header('test'));
	}
	...
	public function testWontBuildHeaderWithArrayInput() {
		...
		Request::from_request()->to_header();
	}

	public function testBuildBaseString() {
		OAuthTestUtils::build_request('POST', 'http://testbed/test', 'n=v');
		$this->assertEquals('POST&http%3A%2F%2Ftestbed%2Ftest&n%3Dv', Request::from_request()->get_signature_base_string());
		
		OAuthTestUtils::build_request('POST', 'http://testbed/test', 'n=v&n=v2');
		$this->assertEquals('POST&http%3A%2F%2Ftestbed%2Ftest&n%3Dv%26n%3Dv2', Request::from_request()->get_signature_base_string());
		
		OAuthTestUtils::build_request('GET', 'http://example.com?n=v');
		$this->assertEquals('GET&http%3A%2F%2Fexample.com&n%3Dv', Request::from_request()->get_signature_base_string());
		...
		$this->assertEquals('POST&https%3A%2F%2Fphotos.example.net%2Frequest_token&oauth_'
							...
							Request::from_request()->get_signature_base_string());									
	...
	public function testBuildSignature() {
		...
		$params .= '&oauth_signature=ignored&oauth_signature_method=HMAC-SHA1';
		OAuthTestUtils::build_request('GET', 'http://photos.example.net/photos?'.$params);			
		$r = Request::from_request();
		
		$cons = new Consumer('key', 'kd94hf93k423kf44');
		$token = new Token('token', 'pfkkdhi9sl3r4s00');
		
		$hmac = new SignatureMethod_HMAC_SHA1();
		$plaintext = new SignatureMethod_PLAINTEXT();
